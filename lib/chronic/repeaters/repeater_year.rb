  ...
  class RepeaterYear < Repeater #:nodoc:
    ...
    def next(pointer)
      ...
      unless @current_year_start
        case pointer
        when :future
          @current_year_start = construct(@now.year + 1)
        when :past
          @current_year_start = construct(@now.year - 1)
        end
      else
        diff = pointer == :future ? 1 : -1
        @current_year_start = construct(@current_year_start.year + diff)
      end

      Span.new(@current_year_start, construct(@current_year_start.year + 1))
    end
    ...
    def this(pointer = :future)
      ...
      case pointer
      when :future
        this_year_start = construct(@now.year, @now.month, @now.day + 1)
        this_year_end = construct(@now.year + 1, 1, 1)
      when :past
        this_year_start = construct(@now.year, 1, 1)
        this_year_end = construct(@now.year, @now.month, @now.day)
      when :none
        this_year_start = construct(@now.year, 1, 1)
        this_year_end = construct(@now.year + 1, 1, 1)
      end
    ...
    def build_offset_time(time, amount, direction)
      ...
      construct(year, time.month, day, time.hour, time.min, time.sec)
    end
