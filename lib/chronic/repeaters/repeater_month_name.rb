  ...
  class RepeaterMonthName < Repeater #:nodoc:
    ...
    def next(pointer)
      ...
      unless @current_month_begin
        ...
        when :future
          if @now.month < index
            @current_month_begin = construct(@now.year, index)
          elsif @now.month > index
            @current_month_begin = construct(@now.year + 1, index)
          end
        when :none
          if @now.month <= index
            @current_month_begin = construct(@now.year, index)
        ...
        case pointer
        when :future
          @current_month_begin = construct(@current_month_begin.year + 1, @current_month_begin.month)
        when :past
          @current_month_begin = construct(@current_month_begin.year - 1, @current_month_begin.month)
        end
      end
      ...
      Span.new(@current_month_begin, construct(next_month_year, next_month_month))
    end
