  ...
  class RepeaterWeek < Repeater #:nodoc:
    ...
    def next(pointer)
      ...
      unless @current_week_start
        case pointer
        when :future
          first_week_day_repeater = RepeaterDayName.new(@repeater_day_name, @options)
    ...
    def this(pointer = :future)
      ...
      case pointer
      when :future
        this_week_start = time_class.local(@now.year, @now.month, @now.day, @now.hour) + RepeaterHour::HOUR_SECONDS
        first_week_day_repeater = RepeaterDayName.new(@repeater_day_name, @options)
        first_week_day_repeater.start = @now
      ...
      when :past
        this_week_end = time_class.local(@now.year, @now.month, @now.day, @now.hour)
        first_week_day_repeater = RepeaterDayName.new(@repeater_day_name, @options)
        first_week_day_repeater.start = @now
        last_span = first_week_day_repeater.next(:past)
